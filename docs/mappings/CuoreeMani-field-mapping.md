...
- **Rows 7–8 are a real gap.** PDC's award vocabulary stops at what was _recommended_ in review — there is no payment/disbursement concept. The nearest fields mean something different; mapping into them would misstate the data. New-base-field candidates: `grant_last_payment_date`, `grant_last_payment_amount`.
