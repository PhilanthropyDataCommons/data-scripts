...
⚠️ Bundle is **synthetic by design** (fake EIN, address, financials). Field _shapes_ transfer; values do not.
...
| ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `README.md` frontmatter `resource:`                     | `organization_website` ⭐ _already parsed, just dropped — one-line fix_                                 |
...
| Candid **PCS codes** (Subject/Population/OrgType) — `civic/0.6`-_required_ | `organization_pcs_subject` / `_population` / `_org_type` |
...
8. ⚠️ **Privacy:** conviction history and recovery status appear only as _aggregate_ population description (publishable). Individual-level equivalents must never enter PDC. 230 of 282 PDC fields are `restricted`; check per field on ingest.
